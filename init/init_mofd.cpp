...
static void configure_zram() {
    ...
    if ((f = fopen(MEMINFO_FILE, "r")) == NULL) {
        LOG(ERROR) << __func__ << ": Failed to open " << MEMINFO_FILE << "\n";
        return;
    }
    ...
    while (fgets(buf, sizeof(buf), f) != NULL) {
        if (strncmp(buf, MEMINFO_KEY, strlen(MEMINFO_KEY)) == 0) {
            ...
            LOG(INFO) << __func__ << ": Found total memory to be " << mem << " kb, zram enabled: " << mode << "\n";
...
void set_feq_values()
{
	...
	if ( strncmp(buf, LOW_CPU, strlen(LOW_CPU)) == 0 ) {
           ...
           LOG(INFO) << __func__ << ": Failed to get max cpu speed: " << buf << "\n";
        }
    }
}
